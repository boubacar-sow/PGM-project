...
def log_gaussian_prob(x, mu, log_sig):
    logprob = (
        -(0.5 * torch.log(2 * torch.pi * torch.ones(1).to(device)) + log_sig)
...
def encoding(enc, x, y, K, use_mean=False):
    mu_qz, log_sig_qz = enc(x, y)
    if use_mean and K>1:
        raise AssertionError("cannot use mean if k>1")
    if use_mean:
        return mu_qz.unsqueeze(0), log_gaussian_prob(
            mu_qz.unsqueeze(0), mu_qz.unsqueeze(0), log_sig_qz.unsqueeze(0)
        )
...
def lowerbound_F(x, y, enc, dec, K=1, IS=False, beta=1.0, use_mean=False):
    # print("Inputs", x.shape, y.shape)
    z, logq = encoding(enc, x, y, K, use_mean)
    # print("Encoding", z.shape, logq.shape)
    ...
    log_pyz = -F.cross_entropy(
        logit_y.flatten(end_dim=1), y_rep.flatten(end_dim=1), reduction="none"
    ).reshape(y_rep.shape[:-1])
...
def bayes_classifier(
    lowerbound, x, enc, dec, dimY, K=1, beta=1.0, use_mean=False
):
    N = x.shape[0]
    logpxy = []
    for i in range(dimY):
        ...
        bound = lowerbound(x, y, enc, dec, K, IS=True, beta=beta, use_mean=use_mean)
...
class BayesModel(nn.Module):
    ...
    def train_model(
        self, model_name, data_loader, optimizer, num_epochs=30, variational_beta=1.0
    ):
        if model_name == "GBZ":
    ...
    def train_gbz(self, data_loader, optimizer, num_epochs=1, variational_beta=1.0):
        print("Traning GBZ")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    def train_dbx(
        self, data_loader, optimizer, num_epochs=30, variational_beta=1.0, K=100
    ):
        pass
